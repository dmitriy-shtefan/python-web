...
Використати той самий список `products` і виконати одне з завдань:

## Завдання (Складність 1)

Написати декоратор `show_calls`, який при кожному виклику функції виводить повідомлення:
`Функцію викликано`
...
7. Написати декоратор print_execution_time, який рахує, скільки секунд виконувалась функція і виводить це число.
```python
import time
t = time.time() # функція time повертає число секунд з 1 січня 1970, 00:00:00 (epoch)
time.sleep(1) # функція sleep зупиняє виконання коду на N секунд (одну в цьому прикладі)
```
...
def get_expensive_products(products):
   time.sleep(5)  # замість повільного запиту до бази даних
